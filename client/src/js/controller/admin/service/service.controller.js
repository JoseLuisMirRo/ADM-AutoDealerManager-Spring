        ...
        content += `
            <tr>
                ...
                <div class="btn-group" role="group" aria-label="Grupo de botones">
                            ...
                            <button type="button" class="btn btn-outline-warning" data-bs-toggle="modal" data-bs-target="#changeServiceStatusModal" data-service-id="${service.id}" data-service-status="${service.status}">
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                // Resetear el formulario y recargar la tabla
                service = {}; // Limpiar el objeto service
...
const updateService = async () => {
    ...
    Swal.fire({
        icon: 'question',
        title: '¿Realizar esta operacion',
        showConfirmButton: true,
        showCancelButton: true
    }).then(async (result) => {
        if (result.isConfirmed) {
            try {
                const response = await fetch(`${URL}/adm/service/${service.id}`, {
                    method: 'PUT',
                    headers: {
                        "Content-Type": "application/json",
                        "Accept": "application/json"
                ...
                const updateServiceModal = bootstrap.Modal.getInstance(modal);
